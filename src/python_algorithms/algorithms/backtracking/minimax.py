...
class BackTrackingMinimax(object):
    def minimax(
        ...
        if is_max:
            return max(
                self.minimax(depth + 1, node_index * 2, False, scores, height),
                self.minimax(depth + 1, node_index * 2 + 1, False, scores, height),
            )

        # If it's the minimizer's turn, choose the minimum score
        # between the two possible moves.
        return min(
            self.minimax(depth + 1, node_index * 2, True, scores, height),
            self.minimax(depth + 1, node_index * 2 + 1, True, scores, height),
        )
