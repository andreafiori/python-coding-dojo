...
class Canny:
    ...
    def suppress_non_maximum(self, image_shape, gradient_direction, sobel_grad):
        """
        ...
        for row in range(1, image_shape[0] - 1):
            for col in range(1, image_shape[1] - 1):
                ...
                if (
                    0 <= direction < self.PI / 8
                    or 15 * self.PI / 8 <= direction <= 2 * self.PI
                    or 7 * self.PI / 8 <= direction <= 9 * self.PI / 8
                ):
    ...
    def canny(self,image, threshold_low=15, threshold_high=30, weak=128, strong=255):
        # gaussian_filter
        gaussian_out = self.img_convolve(image, self.gen_gaussian_kernel(9, sigma=1.4))
        # get the gradient and degree by sobel_filter
        sobel_grad, sobel_theta = self.sobel_filter(gaussian_out)
        gradient_direction = self.PI + np.rad2deg(sobel_theta)

        destination = self.suppress_non_maximum(image.shape, gradient_direction, sobel_grad)

        self.detect_high_low_threshold(
            image.shape, destination, threshold_low, threshold_high, weak, strong
        )

        self.track_edge(image.shape, destination, weak, strong)
