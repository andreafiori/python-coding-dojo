...
def test_example_case():
    X = 5
    A = [1, 3, 1, 4, 2, 3, 5, 4]
    assert FrogRiverOne.solution(X, A) == 6


def test_frog_never_crosses():
    X = 5
    A = [1, 1, 1, 1]  # never covers positions 2,3,4,5
    assert FrogRiverOne.solution(X, A) == -1


def test_all_leaves_same_position():
    X = 3
    A = [2, 2, 2, 2]
    assert FrogRiverOne.solution(X, A) == -1


def test_single_position_success():
    X = 1
    A = [1]
    assert FrogRiverOne.solution(X, A) == 0
...
def test_random_small_case():
    X = 3
    A = [3, 1, 2]
    # positions 1,2,3 all covered at second index 2
    assert FrogRiverOne.solution(X, A) == 2


def test_large_case_permutation():
    X = 5
    A = [5, 4, 3, 2, 1]
    # all positions covered at second index 4
    assert FrogRiverOne.solution(X, A) == 4
